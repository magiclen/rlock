...
impl RLock {
    ...
    /// ```
    #[inline]
    pub async fn acquire_multi_key_read(
        &self,
        keys: impl Into<Arc<Vec<String>>>,
    ) -> Result<RMultiKeyReadLockGuard, AcquireError> {
        self.acquire_multi_key_read_with_options(keys, AcquireOptions::default()).await
    }
    ...
    /// # }
    /// ```
    pub async fn acquire_multi_key_read_with_options(
        &self,
        keys: impl Into<Arc<Vec<String>>>,
        ...
        loop {
            let result = acquire_multi_key_read_lock_async(
                ...
                loop {
                    tokio::select! {
                        ...
                        _ = time::sleep(renew_interval.unwrap_or_else(|| ttl / 2)) => {
                            match renew_multi_key_read_lock_async(&mut connection_manager, &keys, uuid.as_str(), ttl).await {
                                Ok(false) => {
    ...
    /// ```
    #[inline]
    pub async fn acquire_multi_key_write(
        &self,
        keys: impl Into<Arc<Vec<String>>>,
    ) -> Result<RMultiKeyWriteLockGuard, AcquireError> {
        self.acquire_multi_key_write_with_options(keys, AcquireOptions::default()).await
    }
    ...
    /// # }
    /// ```
    pub async fn acquire_multi_key_write_with_options(
        &self,
        keys: impl Into<Arc<Vec<String>>>,
        options: AcquireOptions,
    ) -> Result<RMultiKeyWriteLockGuard, AcquireError> {
        ...
        loop {
            let result = acquire_multi_key_write_lock_async(
                &mut self.inner.connection_manager.clone(),
                &keys,
                uuid.as_ref(),
                ttl,
            ...
            tokio::spawn(async move {
                ...
                loop {
                    tokio::select! {
                        ...
                        _ = time::sleep(renew_interval.unwrap_or_else(|| ttl / 2)) => {
                            match renew_multi_key_write_lock_async(&mut connection_manager, &keys, uuid.as_str(), ttl).await {
                                Ok(false) => {
                                    tracing::trace!(uuid = %uuid, "cannot find the lock, close the renewal task");
                                    break;
                                }
