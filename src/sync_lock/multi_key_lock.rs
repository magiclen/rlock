...
impl RMultiKeyLockGuard {
    ...
    pub fn release(&mut self) -> Result<(), ReleaseError> {
...

        {
            ...
            release_multi_key_lock_sync(&mut *conn, &self.keys, self.uuid.as_str())?;
        }
...
impl RLock {
    ...
    /// ```
    #[inline]
    pub fn acquire_multi_key_mutex(
        &self,
        keys: impl Into<Arc<Vec<String>>>,
    ) -> Result<RMultiKeyLockGuard, AcquireError> {
        self.acquire_multi_key_mutex_with_options(keys, AcquireOptions::default())
    }
    ...
    /// // drop(lock);
    /// ```
    pub fn acquire_multi_key_mutex_with_options(
        &self,
        keys: impl Into<Arc<Vec<String>>>,
        ...
        loop {
            ...
            let result = {
                ...
                acquire_multi_key_lock_sync(&mut *conn, &keys, uuid.as_ref(), ttl)?
            };
                    ...
                    match stop_rx.recv_timeout(renew_interval.unwrap_or_else(|| ttl / 2)) {
                        ...
                        Err(RecvTimeoutError::Timeout) => {
                            let result = pool.checkout().and_then(|mut conn| {
                                renew_multi_key_lock_sync(&mut *conn, &keys, uuid.as_str(), ttl)
                            });
