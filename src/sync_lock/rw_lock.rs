...
impl RReadLockGuard {
    ...
    ///
    /// let mut lock = rlock.acquire_read("rlock:example").unwrap();
    ///
    /// // shared section
    ///
    /// lock.release().unwrap();
    ...
    pub fn release(&mut self) -> Result<(), ReleaseError> {
...

        {
            ...
            release_read_lock_sync(&mut *conn, self.key.as_str(), self.uuid.as_str())?;
        }
...
impl RWriteLockGuard {
    ...
    /// let rlock = RLock::new("redis://127.0.0.1:6379/0").unwrap();
    ///
    /// let mut lock = rlock.acquire_write("rlock:example").unwrap();
    ///
    /// // critial section
    ///
    /// lock.release().unwrap();
    ...
    pub fn release(&mut self) -> Result<(), ReleaseError> {
...

        {
            ...
            release_write_lock_sync(&mut *conn, self.key.as_str(), self.uuid.as_str())?;
        }
...
impl RLock {
    ...
    /// let rlock = RLock::new("redis://127.0.0.1:6379/0").unwrap();
    ///
    /// let lock = rlock.acquire_read("rlock:example").unwrap();
    ///
    /// // shared section
    ///
    /// // drop(lock);
    /// ```
    #[inline]
    pub fn acquire_read(&self, key: impl AsRef<str>) -> Result<RReadLockGuard, AcquireError> {
        self.acquire_read_with_options(key, AcquireOptions::default())
    }
    ...
    /// // drop(lock);
    /// ```
    pub fn acquire_read_with_options(
        &self,
        key: impl AsRef<str>,
        ...
        loop {
            ...
            let result = {
                ...
                acquire_read_lock_sync(&mut *conn, key, uuid.as_ref(), ttl)?
            };
                    ...
                    match stop_rx.recv_timeout(renew_interval.unwrap_or_else(|| ttl / 2)) {
                        ...
                        Err(RecvTimeoutError::Timeout) => {
                            let result = pool.checkout().and_then(|mut conn| {
                                renew_read_lock_sync(&mut *conn, key.as_str(), uuid.as_str(), ttl)
                            });
    ...
    /// let rlock = RLock::new("redis://127.0.0.1:6379/0").unwrap();
    ///
    /// let lock = rlock.acquire_write("rlock:example").unwrap();
    ///
    /// // critial section
    ///
    /// // drop(lock);
    /// ```
    #[inline]
    pub fn acquire_write(&self, key: impl AsRef<str>) -> Result<RWriteLockGuard, AcquireError> {
        self.acquire_write_with_options(key, AcquireOptions::default())
    }
    ...
    /// // drop(lock);
    /// ```
    pub fn acquire_write_with_options(
        &self,
        key: impl AsRef<str>,
        options: AcquireOptions,
        ...
        loop {
            ...
            let result = {
                ...
                acquire_write_lock_sync(&mut *conn, key, uuid.as_ref(), ttl)?
            };
            ...
            thread::spawn(move || {
                ...
                loop {
                    ...
                    match stop_rx.recv_timeout(renew_interval.unwrap_or_else(|| ttl / 2)) {
                        ...
                        Err(RecvTimeoutError::Timeout) => {
                            let result = pool.checkout().and_then(|mut conn| {
                                renew_write_lock_sync(&mut *conn, key.as_str(), uuid.as_str(), ttl)
                            });
